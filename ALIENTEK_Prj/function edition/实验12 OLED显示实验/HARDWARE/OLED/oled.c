...
//num:数值(0~4294967295);	0xFFFFFFFF 		  
...
{         	
	...
	for(t=0;t<len;t++)
	{
		temp=(num/mypow(10,len-t-1))%10;//取三位数字的百位、十位、个位
		if(enshow==0&&t<(len-1))
		{
			if(temp==0)	//判断是否到字符串结尾'\0'
			{
	 	...
	 	OLED_ShowChar(x+(size/2)*t,y,temp+'0',size,1); //显示三位数字的百位、十位、个位
	}
} 
//显示字符串
//x,y:起点坐标  
